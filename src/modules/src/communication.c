...
void communicationInit() {
  DEBUG_PRINT("Communication module init\n");
...
void createKoopmanPacket(uart_packet* packet, va_list* args) {
  const fullState* state = va_arg(*args, fullState*);

  ASSERT(state != NULL);

  uint8_t payloadLength = sizeof(*state);
  ASSERT(payloadLength <= MAX_PAYLOAD_LENGTH);

  packet->start = START_BYTE;  
  packet->serviceType = KOOPMAN_PACKET;
  packet->payloadLength = payloadLength;
  ...
  DEBUG_PRINT("Koopman packt setn %f\n", (double)state->roll);
...
void sendDataUART(const char *format, ...) {
  ...
  while (*format != '\0') {
    switch (*format++) {
      ...
      case 'K': { 
        // Forward (currently dummy values) 
        createKoopmanPacket(&packet, &args);
        break;
      }
      default:
        break;
